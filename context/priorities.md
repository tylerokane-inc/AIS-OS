...
2. **Auto-organizing Obsidian pipeline** — raw notes land in an inbox, get
   ...
   bloat).
3. **Day-off resilience test** — the system runs well enough that a full day
   off the phone doesn't break anything. On return, missed tasks are
   tracked, rescheduled, and rolled forward automatically. Pick back up, no
   dropped threads.
