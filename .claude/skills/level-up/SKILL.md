---
name: level-up
description: Use weekly to find and ship one new automation. Walks the Operator Loop interview — Think (find the candidate) → Decide (decide on one) → Build (build it). Trigger on "let's level up", "what should I automate next", "find me leverage this week", or as a Friday ritual. One run = one shipped artifact.
---

## What this skill does

Walks the user through the Operator Loop each week to surface and ship one new automation. **One interview = one artifact.** It also installs the Operator Loop into the user's head over time — after 4-6 runs, the user starts spotting opportunities mid-week without prompting because the questions have become internal defaults.
...
### Phase 1 — Think phase (find the candidate)
...
Quote relevant Think principles when they fit:
...
**Output of Phase 1:** numbered list of 1-3 candidate opportunities, one-line "why this is leverage" per candidate. Ask: *"Pick one to decide on."*

### Phase 2 — Decide phase (decide on one)

User picks one candidate. Walk the 5-step Decide pipeline:
...
**Output of Phase 2:** decided automation spec written to `decisions/log.md` as a dated entry with all five answers + autonomy level + KPI. Durable record of what was decided and why.

### Phase 3 — Build handoff (build it)
...
1. **One `decisions/log.md` entry** — dated, with the Decide-phase spec
2. **One scaffolded artifact** — prompt, skill, or agent file
3. **A one-screen close** — what was decided, what was built, and the Bike Method Phase 1 reminder

## Critical implementation rules

1. **One interview = one artifact.** No juggling multiple candidates in one run.
2. **Think phase always runs first.** Even if user comes in with a pre-formed idea.
...
5. **Boring-is-Beautiful default in Build handoff.** Default = highest non-AI option.
...
- **Dry run with no prompt.** Expected: skill surfaces 2-3 candidates pulled from the user's own recent activity, priorities, and top_pain. Generic output ("you should build a brief") = fail.
